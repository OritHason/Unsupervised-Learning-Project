...
def plot_figure_2():
    ...
    gmm_labels = gmm(data=processed_data_no_remotejobwork, n_clusters=3)
    for feature,ax in zip(remote_job_work,axes[:3]):
        labels = data[feature]
        merged_data = pd.DataFrame({'cluster_labels': gmm_labels, 'other_labels': labels})
    ...
    for i in clusters:
        siloute_all_pca.append(get_avg_siloute_score(X=reduced_data_pca, n_clusters=i,method='gmm'))
        siloute_numeric_pca.append(get_avg_siloute_score(X=numeric_pca, n_clusters=i,method='gmm')[0])
        siloute_all_no_pca.append(get_avg_siloute_score(X=processed_data, n_clusters=i,method='gmm'))
        siloute_numeric_no_pca.append(get_avg_siloute_score(X=numeric_df, n_clusters=i,method='gmm')[0])
    ...
    plot_dual_silhouettes_from_values(axes[5], siloute_samples_3_all_pca, siloute_samples_3_all_no_pca,
                                       clusters_3_pca, clusters_3_no_pca, 3)
    if not os.path.exists('Figures'):
        try:
            os.makedirs('Figures')
        except OSError as e:
            print(f"Error creating directory: {e}")
    output_path = os.path.join('Figures', 'Fig_2.pdf')
